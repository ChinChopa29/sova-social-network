...
const fetchCategory = async () => {
  try {
    ...
    const res = await axiosClient.get(`api/categories/${slug}`);
...
<template>
  ...
  <div v-else class="p-4">
    ...
    <h1 class="text-xl font-bold mb-2">{{ category.name }}</h1>
    <p class="text-gray-600">ID: {{ category.id }}</p>
    <p class="text-gray-600">Создан: {{ formatDate(category.created_at) }}</p>
    <p class="text-gray-600">Обновлён: {{ formatDate(category.updated_at) }}</p>
  </div>
</template>
